...
tag = "quantile_alpha" if use_disparity else "similarity"
top_countries = 100
...
def make_graph(df, use_disparity=True, param=0.3):
    ...
    if use_disparity:
        ...
        G_top = nx.Graph(((u, v, d) for u, v, d in G.edges(data=True) if d.get("alpha") <= alpha_thresh))
        G_top.add_nodes_from(G.nodes())  # preserve singletons
...
def prepare_similarity_matrix(df, nodes=None, fill_diag=1.0):
    ...
    countries = sorted(set(df["country1"]).union(df["country2"]))
...
for param in thresholds:
    ...
    for dimension in ["all"]:
        ...
        G_culture = nx.read_gexf(f"../data/network_top_{top_countries}_countries_culture_top{param}_{tag}_edges.gexf")
        nodes_to_keep = sorted(G_culture.nodes())
        
        # Image similarities
        sim_images = pd.read_csv(f"../data/all_average_sims_top_{top_countries}_countries.csv")
        sim_images_mat = prepare_similarity_matrix(sim_images, nodes=nodes_to_keep)
        G_image = make_graph(sim_images_mat, use_disparity, param)

        sim_images_mat_all = prepare_similarity_matrix(sim_images)
        G_image_all = make_graph(sim_images_mat_all, use_disparity, param)
        ...
        sim_lang_mat = prepare_similarity_matrix(pd.DataFrame(expanded_rows), nodes=nodes_to_keep)
        G_language = make_graph(sim_lang_mat, use_disparity, param)

        sim_lang_mat_all = prepare_similarity_matrix(pd.DataFrame(expanded_rows))
        G_language_all = make_graph(sim_lang_mat_all, use_disparity, param)

        # Save networks
        for g, name in zip([G_culture, G_image, G_language], ["culture","image","language"]):
            nx.write_gexf(g, f"../data/network_{name}_top{param}_{tag}_topcountries{top_countries}.gexf")

        # Save networks
        for g, name in zip([G_image_all, G_language_all], ["image","language"]):
            nx.write_gexf(g, f"../data/network_{name}_all_top{param}_{tag}_topcountries{top_countries}.gexf")

        # Metrics
        results = {
            "dimension": dimension,
            "n_edges_culture": G_culture.number_of_edges(),
            "n_edges_image": G_image.number_of_edges(),
            "n_edges_language": G_language.number_of_edges(),
            "edge_jaccard_culture_image": edge_jaccard(G_culture, G_image),
            "edge_jaccard_culture_language": edge_jaccard(G_culture, G_language),
            "node_jaccard_culture_image": avg_neighborhood_jaccard(G_culture, G_image),
            "node_jaccard_culture_language": avg_neighborhood_jaccard(G_culture, G_language),
            "community_culture_image": average_best_match_jaccard(list(nx.algorithms.community.greedy_modularity_communities(G_culture)),
                                            list(nx.algorithms.community.greedy_modularity_communities(G_image)))[0],
            "community_culture_language": average_best_match_jaccard(list(nx.algorithms.community.greedy_modularity_communities(G_culture)),
                                                list(nx.algorithms.community.greedy_modularity_communities(G_language)))[0]
        }
        list_results.append(results)

    pd.DataFrame(list_results).to_csv(f"../data/comparison_culture_networks_top{param}_{tag}_topcountries{top_countries}.csv", index=False)
