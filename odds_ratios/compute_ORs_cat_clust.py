...
for category in tqdm(categories["category"], desc="Processing categories"):
    ...
    # cluster assignments
    df_top_clusters = pd.read_csv(f"../data/umap_files/{category}_umap_clusters.csv")
    ...
    if "countrycode" in df_top_clusters.columns:
        # drop countrycode column if it exists
        df_top_clusters = df_top_clusters.drop(columns=["countrycode"])
    ...
    if not os.path.exists(f"../results/similarity_matrices/clusterable"):
        os.makedirs(f"../results/similarity_matrices/clusterable") 
    np.save(f"../results/similarity_matrices/clusterable/{category}_similarity_matrix_log_euclidean.npy", similarity_matrix)

    with open(f"../results/similarity_matrices/clusterable/{category}_countries.pkl", "wb") as f:
